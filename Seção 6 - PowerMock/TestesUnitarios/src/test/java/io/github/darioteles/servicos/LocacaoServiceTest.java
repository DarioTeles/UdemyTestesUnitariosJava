...
public class LocacaoServiceTest {
	...
	public void setup() {
		...
		//LocacaoDAO dao = new LocacaoDaoFake(); Exemplo de Fake Object (Implementação LocacaoDaoFake). Refatorado para utilizar Mock em vez de Object Fake.
		//
	...
	public void deveAlugarFilme() throws Exception {
		...
		//Assume.assumeFalse(DataUtils.verificarDiaSemana(new Date(), Calendar.SATURDAY)); //Testar dias diferentes de sábado //refatorado ao utilizar PowerMock para mockar construtor
...
//	}
	...
	public void deveDevolverNaSegundaAoAlugarNoSabado() throws Exception {
		...
		//Cenario
		//Assume.assumeTrue(DataUtils.verificarDiaSemana(new Date(), Calendar.SATURDAY)); //Testar somente vendas ao sábado //Refatorado ao utilizar PowerMock para mockar construtor.
		Usuario usuario = UsuarioBuilder.umUsuario().agora();
		List<Filme> filmes = Arrays.asList(FilmeBuilder.umFilme().agora());		
		PowerMockito.whenNew(Date.class).withNoArguments().thenReturn(DataUtils.obterData(29, 4, 2017)); //Mockando construtor da classe Date.
	...
	public void deveEnviarEmailParaAlocacoesAtrasadas(){
		...
		Mockito.verify(emailService, Mockito.times(2)).notificarAtraso(Mockito.any(Usuario.class)); //Forma mais genérica
